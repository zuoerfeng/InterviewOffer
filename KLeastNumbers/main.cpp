...
// ====================方法2====================
//利用桶的思想，每次只用对桶元素进行查找最大值，增加删除操作，时间复杂度降到O(nlgk)
//查找删除增加操作的复杂度为lgn，这可以用最大堆和红黑树来完成，可以用STL的红黑树来完成
typedef multiset<int, greater<int>> intSet;   //greater表示容器中按照升序排列
